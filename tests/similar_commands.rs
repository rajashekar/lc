...
mod similar_search_tests {
    ...
    fn setup_test_database_with_vectors() -> VectorDatabase {
        ...
        let test_data = vec![
            ...
            ("Machine learning is a subset of artificial intelligence", vec![0.8, 0.6, 0.2, 0.1, 0.0]),
            ("Deep learning uses neural networks with multiple layers", vec![0.7, 0.7, 0.3, 0.1, 0.0]),
            ("Artificial intelligence enables computers to think", vec![0.9, 0.5, 0.1, 0.1, 0.0]),
            
            // Programming related (different cluster)
            ("Python is a popular programming language", vec![0.1, 0.2, 0.8, 0.7, 0.1]),
            ("JavaScript runs in web browsers", vec![0.0, 0.1, 0.9, 0.6, 0.2]),
            
            // Completely different topic
            ("Cooking pasta requires boiling water", vec![0.0, 0.0, 0.1, 0.1, 0.9]),
        ];
    ...
    fn test_basic_similarity_search() {
        ...
        assert!(similar[0].0.text.to_lowercase().contains("artificial") || 
    ...
    fn test_similarity_search_ordering() {
        ...
        let ai_keywords = ["machine", "learning", "artificial", "intelligence", "deep", "neural"];
        
        for (entry, _) in top_results {
            let text_lower = entry.text.to_lowercase();
            let has_ai_keyword = ai_keywords.iter().any(|&keyword| text_lower.contains(keyword));
            assert!(has_ai_keyword, "Top result should contain AI-related keywords: {}", entry.text);
        }
    }
    ...
    fn test_similarity_search_single_vector() {
        ...
        db.add_vector("Single vector", &vector, model, provider).unwrap();
...
mod similar_model_resolution_tests {
    ...
    fn test_similar_model_resolution_from_database() {
        ...
        db.add_vector("Test text", &vector, stored_model, stored_provider).unwrap();
        ...
        let result = lc::cli::resolve_model_and_provider(&config, Some(db_provider), Some(db_model));
        assert!(result.is_ok());
    ...
    fn test_similar_model_override() {
        ...
        db.add_vector("Test", &vec![0.1, 0.2, 0.3], "text-embedding-3-small", "openai").unwrap();
...
mod similar_parameter_validation_tests {
    ...
    fn test_similarity_limit_validation() {
        ...
        for limit in test_limits {
            ...
            assert_eq!(similar.len(), expected_len, "Wrong result count for limit: {}", limit);
        }
    ...
    fn test_similarity_query_vector_validation() {
        ...
        db.add_vector("Test vector", &stored_vector, model, provider).unwrap();
    ...
    fn test_similarity_with_special_values() {
        ...
        db.add_vector("Normal vector", &normal_vector, model, provider).unwrap();
            ...
            match result {
                Ok(similar) => {
                    ...
                    if !similar.is_empty() {
                        ...
                        assert!(similar[0].1.is_finite(), "Similarity score should be finite for query: {:?}", query);
                    }
                }
                Err(_) => {
...
mod similar_error_handling_tests {
    ...
    fn test_similar_with_corrupted_data() {
        ...
        db.add_vector("Normal text", &vec![0.1, 0.2, 0.3], model, provider).unwrap();
    ...
    fn test_similar_with_invalid_model_info() {
        ...
        let result = lc::cli::resolve_model_and_provider(&config, Some("invalid".to_string()), Some("model".to_string()));
        assert!(result.is_err());
    }
}
...
mod similar_performance_tests {
    ...
    fn test_similarity_search_performance() {
        ...
        for i in 0..vector_count {
            ...
            db.add_vector(&format!("Vector {}", i), &vector, model, provider).unwrap();
        }
    ...
    fn test_similarity_search_with_large_vectors() {
        ...
        for i in 0..vector_count {
            let vector: Vec<f64> = (0..dimension).map(|j| ((i * dimension + j) as f64) * 0.0001).collect();
            db.add_vector(&format!("Large vector {}", i), &vector, model, provider).unwrap();
        }
...
mod similar_integration_tests {
    ...
    fn test_complete_similarity_workflow() {
        ...
        assert!(similar[1].0.text.to_lowercase().contains("machine") || 
        ...
        assert!(web_similar[0].0.text.to_lowercase().contains("web") || 
    ...
    fn test_similarity_with_model_consistency() {
        ...
        db.add_vector("First text", &vec![0.1, 0.2, 0.3], model, provider).unwrap();
        db.add_vector("Second text", &vec![0.2, 0.3, 0.4], model, provider).unwrap();
