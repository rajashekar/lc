use lc::cli::{Cli, resolve_model_and_provider, read_and_format_attachments, is_code_file};
...
fn create_comprehensive_config() -> Config {
    ...
    config.aliases.insert("smart".to_string(), "anthropic:claude-3".to_string());
    
    // Add templates
    config.templates.insert("code".to_string(), "Explain this code: ".to_string());
    config.templates.insert("review".to_string(), "Review this code for bugs: ".to_string());
...
mod prompt_model_resolution_tests {
    ...
    fn test_resolve_model_nonexistent_provider() {
        ...
        assert!(result.unwrap_err().to_string().contains("Provider 'nonexistent' not found"));
    }
    ...
    fn test_resolve_model_no_default_provider() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("No default provider configured"));
    }
    ...
    fn test_resolve_model_no_default_model() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("No default model configured"));
    }

    #[test]
    fn test_resolve_model_invalid_alias_format() {
        let mut config = create_test_config();
        config.aliases.insert("invalid".to_string(), "just-model-name".to_string());
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid alias target format"));
    }
}
...
mod prompt_error_handling_tests {
    ...
    fn test_resolve_model_with_invalid_provider_model_format() {
        ...
        let result = resolve_model_and_provider(
            &config, 
            None, 
            Some("provider:model:extra".to_string())
        );
...
mod prompt_integration_tests {
    ...
    fn test_prompt_workflow_parameter_precedence() {
        ...
        // Parse CLI overrides
        let parsed_max_tokens = cli_max_tokens.as_ref()
            .map(|s| Config::parse_max_tokens(s).unwrap());
        let parsed_temperature = cli_temperature.as_ref()
...
mod prompt_attachment_edge_cases_tests {
    ...
    fn test_code_file_extensions_comprehensive() {
        ...
        for ext in code_extensions {
            assert!(is_code_file(ext), "Extension '{}' should be recognized as code", ext);
        }
    }
    ...
    fn test_non_code_file_extensions() {
        let non_code_extensions = vec![
            ...
            "png", "jpg", "jpeg", "gif", "bmp", "svg", "mp3", "mp4", "avi",
            "zip", "tar", "gz", "rar", "exe", "dll", "so", "dylib"
        ];
        
        for ext in non_code_extensions {
            assert!(!is_code_file(ext), "Extension '{}' should not be recognized as code", ext);
        }
    }
}
...
mod prompt_comprehensive_integration_tests {
    ...
    fn test_prompt_error_recovery_scenarios() {
        ...
        config_with_bad_alias.aliases.insert("bad".to_string(), "invalid-format".to_string());
        ...
        let bad_attachments = vec!["nonexistent1.txt".to_string(), "nonexistent2.py".to_string()];
    ...
    fn test_prompt_performance_edge_cases() {
        ...
        for i in 0..100 {
            config_many_aliases.aliases.insert(
                format!("alias{}", i),
                format!("openai:model{}", i)
            );
        }
        ...
        for i in 0..100 {
            config_many_templates.templates.insert(
                format!("template{}", i),
                format!("Template content {}", i)
            );
        }
