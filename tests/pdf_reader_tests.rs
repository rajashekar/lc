#[cfg(feature = "pdf")]
use lc::readers::{FileReader, pdf::PdfReader};
#[cfg(not(feature = "pdf"))]
...
mod tests {
    ...
    fn test_corrupted_pdf_handling() -> Result<()> {
        ...
        #[cfg(feature = "pdf")]
        {
            ...
            assert!(
                error_msg.contains("Failed to extract text from PDF") ||
    ...
    fn test_fixture_simple_text_pdf() {
        ...
        if Path::new(fixture_path).exists() {
            ...
            match result {
                ...
                Err(e) => {
                    println!("Could not extract text from fixture (this may be expected): {}", e);
