...
use lc::proxy::{
    ProxyState, ProxyModelsQuery, ProxyModelsResponse, ProxyModel,
    ProxyChatRequest, ProxyChatResponse, ProxyChoice, ProxyUsage,
    parse_model_string, generate_api_key
};
...
mod proxy_model_parsing_tests {
    ...
    fn test_parse_model_invalid_alias() {
        let mut config = create_config_with_aliases();
        config.aliases.insert("invalid".to_string(), "just-a-model".to_string());
...
mod proxy_filtering_tests {
    ...
    fn test_model_filter_logic() {
        ...
        for model_request in model_requests {
            if let Some(ref model_filter) = state.model_filter {
                let should_pass = model_request.contains(model_filter) || 
                                model_request.ends_with(model_filter);
                
                if model_request == "gpt-4" || model_request == "openai:gpt-4" {
                    assert!(should_pass, "Model {} should pass filter {}", model_request, model_filter);
                } else {
                    assert!(!should_pass, "Model {} should not pass filter {}", model_request, model_filter);
                }
            }
        }
    }
    ...
    fn test_combined_filters() {
        let state = create_test_state_with_filters(
        ...
        let provider_passes = state.provider_filter.as_ref().map_or(true, |f| provider == *f);
...
mod proxy_validation_tests {
    ...
    fn test_proxy_models_query_validation() {
        ...
        let valid_providers = vec!["openai", "anthropic", "cohere", "provider-name", "provider_name"];
    ...
    fn test_proxy_parameter_ranges() {
        ...
        for temp in valid_temperatures {
            assert!(temp >= 0.0 && temp <= 2.0, "Temperature {} should be valid", temp);
        }
        
        for temp in invalid_temperatures {
            assert!(!(temp >= 0.0 && temp <= 2.0), "Temperature {} should be invalid", temp);
        }
    ...
    fn test_proxy_model_id_validation() {
        ...
        for model_id in valid_model_ids {
            assert!(!model_id.is_empty(), "Model ID {} should be valid", model_id);
            if model_id.contains(':') {
                let parts: Vec<&str> = model_id.split(':').collect();
                assert!(parts.len() >= 2, "Model ID {} should have provider and model", model_id);
                assert!(!parts[0].is_empty(), "Provider in {} should not be empty", model_id);
                assert!(!parts[1].is_empty(), "Model in {} should not be empty", model_id);
            }
        }
