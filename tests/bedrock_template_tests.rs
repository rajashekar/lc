...
"#;
    ...
    processor.register_template(template).expect("Failed to register template");
    ...
    assert_eq!(
        second_message.get("role").unwrap().as_str().unwrap(),
        "assistant"
    );
...
"#;
    ...
    assert_eq!(
        messages[2].get("role").unwrap().as_str().unwrap(),
        "assistant"
    );
}
...
"#;
    ...
    assert_eq!(
        inference_config.get("maxTokens").unwrap().as_u64().unwrap(),
        500
    );
    assert_eq!(
        inference_config
