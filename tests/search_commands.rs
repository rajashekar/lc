...
fn test_search_provider_list() -> Result<()> {
    ...
    assert!(output.status.success(), "Failed to list providers: {}", String::from_utf8_lossy(&output.stderr));
    
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("brave_list_test"), "Provider name not found in list output");
    assert!(stdout.contains("https://api.search.brave.com/res/v1"), "Provider URL not found in list output");
...
fn test_search_provider_delete() -> Result<()> {
    ...
    // Delete the provider
    let output = Command::new("cargo")
        .args(&["run", "--", "search", "provider", "delete", "brave_delete_test"])
        .output()?;
    
    assert!(output.status.success(), "Failed to delete provider: {}", String::from_utf8_lossy(&output.stderr));
...
fn test_default_search_provider_config() -> Result<()> {
    ...
    assert!(output.status.success(), "Failed to set default provider: {}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
    ...
    assert!(output.status.success(), "Failed to get default provider: {}", String::from_utf8_lossy(&output.stderr));
    let stdout = String::from_utf8_lossy(&output.stdout);
...
fn test_search_query_missing_provider() -> Result<()> {
    ...
    assert!(stderr.contains("Search provider 'nonexistent' not found") || stderr.contains("not found"));
...
fn test_search_integration_flag() -> Result<()> {
    ...
    assert!(stderr.contains("Search failed") || stderr.contains("API") || stderr.contains("401") ||
            stderr.contains("not found") || stderr.contains("No API key"));
