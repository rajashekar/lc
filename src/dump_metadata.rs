...
impl MetadataDumper {
    ...
    pub async fn dump_all_metadata() -> Result<()> {
        ...
        println!("{} Dumping fresh raw metadata for all configured providers...", "🔍".blue());
        ...
        if !models_raw_dir.exists() {
            ...
            println!("{} Created directory: {}", "📁".blue(), models_raw_dir.display());
        }
        
        println!("{} Raw models directory: {}", "📁".blue(), models_raw_dir.display());
        ...
        for (provider_name, provider_config) in sorted_providers {
            ...
            println!("{} Fetching fresh models from {}...", "📡".blue(), provider_name);
    ...
    pub async fn dump_provider_by_name(provider_name: &str) -> Result<()> {
        ...
        println!("{} Dumping fresh raw metadata for provider: {}", "🔍".blue(), provider_name);
        ...
        match Self::fetch_and_save_raw_metadata(&config, provider_name, &models_raw_dir).await {
            Ok(_) => {
                println!("\n{} Successfully dumped fresh raw metadata for {}", "✅".green(), provider_name);
            }
            Err(e) => {
    ...
    pub async fn list_cached_metadata() -> Result<()> {
        ...
        if !models_raw_dir.exists() {
            println!("No models_raw directory found at: {}", models_raw_dir.display());
        ...
        for (provider_name, path, size) in files {
            println!("  {} {} - {} ({})", "•".blue(), provider_name, path.display(), size);
        }
        
        println!("\n{} Use 'lc dump <provider>' to fetch fresh raw data for a specific provider", "💡".yellow());
