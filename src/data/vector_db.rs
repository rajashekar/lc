...
// Optimized cosine similarity calculation leveraging Rust's autovectorization via iterators
pub fn cosine_similarity_simd(a: &[f64], b: &[f64]) -> f64 {
    ...
    // Using .iter().zip() enables LLVM to completely elide bounds checks
    // and reliably apply automatic SIMD vectorization and loop unrolling.
...
pub fn cosine_similarity_precomputed(a: &[f64], b: &[f64], norm_a: f64) -> f64 {
    ...
    // Using .iter().zip() enables reliable autovectorization
    for (av, bv) in a.iter().zip(b.iter()) {
        dot_product += av * bv;
        norm_b_sq += bv * bv;
    }
