...
impl MultiLineInput {
            ...
            if let Event::Key(key_event) = event {
                ...
                if key_event.kind == KeyEventKind::Press {
                    match self.handle_key_event(key_event)? {
                        ...
                        InputAction::Cancel => {
                            // Print ^C for visual feedback like standard Unix terminals
                            print!("^C");
    ...
    fn handle_key_event(&mut self, key_event: KeyEvent) -> Result<InputAction> {
        ...
        match key_event.code {
            ...
            KeyCode::Char(c) => {
                // Convert cursor_pos (char index) to byte index
                let byte_idx = self
                    .current_line
                    .char_indices()
                    .nth(self.cursor_pos)
                ...
                let rest = &self.current_line[byte_idx + c.len_utf8()..];
                if !rest.is_empty() {
                    print!("{}", rest);
            ...
            KeyCode::Backspace => {
                if self.cursor_pos > 0 {
                    // Convert cursor_pos - 1 (char index) to byte index
                    let byte_idx = self
                        .current_line
                        .char_indices()
