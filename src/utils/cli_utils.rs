...
pub fn is_code_file(ext: &str) -> bool {
    let code_extensions: HashSet<&str> = [
        "rs", "py", "js", "ts", "java", "cpp", "c", "h", "hpp", "go", "rb", "php", "swift",
        "kt", "scala", "sh", "bash", "zsh", "fish", "ps1", "bat", "cmd", "html", "css", "scss",
        "sass", "less", "xml", "json", "yaml", "yml", "toml", "ini", "cfg", "conf", "sql", "r",
        "m", "mm", "pl", "pm", "lua", "vim", "dockerfile", "makefile", "cmake", "gradle", "maven"
    ].iter().cloned().collect();
            ...
            if m.contains(':') && !has_provider_override {
                ...
                if parts.len() == 2 {
                    ...
                    if !config.providers.contains_key(&alias_provider) {
                        return Err(anyhow!("Provider '{}' not found in configuration", alias_provider));
                    }
            ...
            if !has_provider_override {
                if let Some(alias_target) = config.aliases.get(&m) {
                    ...
                    if parts.len() == 2 {
                        ...
                        // Verify provider exists
                        if !config.providers.contains_key(&alias_provider) {
                            return Err(anyhow!("Provider '{}' from alias not found in configuration", alias_provider));
                        }
                        
                        return Ok((alias_provider, alias_model));
                    } else {
                        return Err(anyhow!("Invalid alias target format: '{}'. Expected 'provider:model'", alias_target));
                    }
                }
            }
