...
impl JinaProvider {
    ...
    pub async fn search(&self, query: &str, count: Option<usize>) -> Result<SearchResults> {
        ...
        crate::debug_log!(
            "Jina: Making GET request to {} with params: {:?}",
            self.url,
            params
        );
        ...
        if want_json {
            ...
            if use_full_content {
                ...
                let direct_response: JinaDirectResponse = serde_json::from_str(&response_text)
                    .map_err(|e| {
                        anyhow::anyhow!("Failed to parse Jina direct JSON response: {}", e)
                    })?;
            ...
            for line in lines {
                ...
                if let Some(title_match) = line.strip_prefix("[") {
                    // BOLT-PERF: Use split_once instead of find to prevent double string scanning
                    if let Some((_, title_text)) = title_match.split_once("] Title: ") {
                ...
                if let Some(url_match) = line.strip_prefix("[") {
                    // BOLT-PERF: Use split_once instead of find to prevent double string scanning
                    if let Some((_, url_text)) = url_match.split_once("] URL Source: ") {
                        let url = url_text.to_string();
                ...
                if let Some(desc_match) = line.strip_prefix("[") {
                    // BOLT-PERF: Use split_once instead of find to prevent double string scanning
                    if let Some((_, desc_text)) = desc_match.split_once("] Description: ") {
                        let description = desc_text.to_string();
...
pub async fn search(
    ...
    let provider = JinaProvider::new(provider_config.url.clone(), provider_config.headers.clone());
