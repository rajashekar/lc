...
impl DuckDuckGoProvider {
    ...
    pub async fn search(&self, query: &str, count: Option<usize>) -> Result<SearchResults> {
        ...
        // ⚡ Bolt: Use into_iter() and in-place string manipulation to avoid redundant allocations
            ...
            .take(max_results.saturating_sub(results.len()))
        {
            if !result.text.is_empty() && !result.first_url.is_empty() {
                ...
                // ⚡ Bolt: Use split_off and truncate to reduce allocations from two to one by reusing capacity
                ...
                .take(max_results.saturating_sub(results.len()))
            {
                if let (Some(mut text), Some(url)) = (topic.text, topic.first_url) {
                    if !text.is_empty() && !url.is_empty() {
                        ...
                        // ⚡ Bolt: Apply same split_off/truncate optimization for related topics
