    ...
    pub async fn search(&self, query: &str, count: Option<usize>) -> Result<SearchResults> {
            ...
            .take(max_results.saturating_sub(results.len()))
        {
            if !result.text.is_empty() && !result.first_url.is_empty() {
                ...
                // We use into_iter() to take ownership of the String allocations in results.
                // Using split_off() and truncate() reuses the existing capacity and prevents
                // allocating new strings when splitting the text into title and snippet.
                ...
                .take(max_results.saturating_sub(results.len()))
            {
                if let (Some(mut text), Some(url)) = (topic.text.take(), topic.first_url.take()) {
                    if !text.is_empty() && !url.is_empty() {
                        ...
                        // PERFORMANCE OPTIMIZATION:
                        // As above, we split_off and truncate to avoid additional String allocations.
