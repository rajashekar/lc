...
impl GeminiClient {
    ...
    pub async fn chat(&self, request: &GeminiChatRequest, model: &str) -> Result<String> {
        ...
        let url = if let Some(ref config) = self.provider_config {
            ...
            crate::debug_log!(
                "GeminiClient: Using provider config URL generation, generated URL: {}",
                generated_url
            );
            generated_url
        } else {
            ...
            crate::debug_log!(
                "GeminiClient: Using fallback URL generation, generated URL: {}",
                fallback_url
            );
            fallback_url
        };
        ...
        if let Some(candidate) = gemini_response.candidates.first() {
            ...
            for part in &candidate.content.parts {
                if let GeminiResponsePart::FunctionCall { function_call } = part {
                    ...
                    response
                        .push_str("*Tool calls detected - execution handled by chat module*\n\n");
                    return Ok(response);
                }
            }
    ...
    pub async fn chat_with_tools(
        ...
        let url = if let Some(ref config) = self.provider_config {
            let generated_url = config.get_chat_url(model);
            crate::debug_log!(
                "GeminiClient: Using provider config URL generation for tools, generated URL: {}",
                generated_url
            );
            generated_url
        } else {
            ...
            let fallback_url = format!("{}{}", self.base_url, chat_path);
            crate::debug_log!(
                "GeminiClient: Using fallback URL generation for tools, generated URL: {}",
                fallback_url
            );
            fallback_url
        };
